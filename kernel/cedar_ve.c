...
static irqreturn_t VideoEngineInterupt(int irq, void *dev)
{
	...
	if (modual_sel & (3 << 6)) {
		...
		{
			ve_int_status_reg =
			    (unsigned int)(addrs.regs_macc + 0xb00 + 0x1c);
			ve_int_ctrl_reg =
			    (unsigned int)(addrs.regs_macc + 0xb00 + 0x14);
		...
		{
			ve_int_status_reg =
			    (unsigned int)(addrs.regs_macc + 0xa00 + 0x10);
			ve_int_ctrl_reg =
			    (unsigned int)(addrs.regs_macc + 0xa00 + 0x08);
		...
		if (status && interrupt_enable)	//modify by fangning 2013-05-22
		{
			...
			{
				ve_int_ctrl_reg =
				    (unsigned int)(addrs.regs_macc + 0xb00 +
						   0x14);
			...
			{
				ve_int_ctrl_reg =
				    (unsigned int)(addrs.regs_macc + 0xa00 +
						   0x08);
				val = readl(ve_int_ctrl_reg);
	...
	if (modual_sel <= 4) {
		...
		case 0:	//mpeg124  
			ve_int_status_reg =
			    (unsigned int)(addrs.regs_macc + 0x100 + 0x1c);
			ve_int_ctrl_reg =
			    (unsigned int)(addrs.regs_macc + 0x100 + 0x14);
			interrupt_enable = readl(ve_int_ctrl_reg) & (0x7c);
			break;
		case 1:	//h264    
			ve_int_status_reg =
			    (unsigned int)(addrs.regs_macc + 0x200 + 0x28);
			ve_int_ctrl_reg =
			    (unsigned int)(addrs.regs_macc + 0x200 + 0x20);
			interrupt_enable = readl(ve_int_ctrl_reg) & (0xf);
			break;
		case 2:	//vc1  
			ve_int_status_reg =
			    (unsigned int)(addrs.regs_macc + 0x300 + 0x2c);
			ve_int_ctrl_reg =
			    (unsigned int)(addrs.regs_macc + 0x300 + 0x24);
			interrupt_enable = readl(ve_int_ctrl_reg) & (0xf);
			break;
		case 3:	//rmvb      
			ve_int_status_reg =
			    (unsigned int)(addrs.regs_macc + 0x400 + 0x1c);
			ve_int_ctrl_reg =
			    (unsigned int)(addrs.regs_macc + 0x400 + 0x14);
			interrupt_enable = readl(ve_int_ctrl_reg) & (0xf);
			break;

		case 4:	//hevc          
			ve_int_status_reg =
			    (unsigned int)(addrs.regs_macc + 0x500 + 0x38);
			ve_int_ctrl_reg =
			    (unsigned int)(addrs.regs_macc + 0x500 + 0x30);
			interrupt_enable = readl(ve_int_ctrl_reg) & (0xf);
			break;
...
static void cedar_engine_for_timer_rel(unsigned long arg)
{
	...
	if (list_empty(&run_task_list)) {
		...
		printk
		    ("Warring: cedar engine timeout for clk disable, but task left, something wrong?\n");
...
static void cedar_engine_for_events(unsigned long arg)
{
	...
	if (!list_empty(&run_task_list)) {
		task_entry =
		    list_entry(run_task_list.next, struct cedarv_engine_task,
			       list);
...
long cedardev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_ENGINE_REQ:
		...
		task_ptr =
		    kmalloc(sizeof(struct cedarv_engine_task), GFP_KERNEL);
		if (!task_ptr) {
	...
	case IOCTL_WAIT_VE_DE:
		...
		wait_event_interruptible_timeout(wait_ve,
						 cedar_devp->de_irq_flag,
						 ve_timeout * HZ);
	...
	case IOCTL_SET_VE_FREQ:
		{
			...
			if (arg_rate >= VE_CLK_LOW_WATER &&
				...
				if (!clk_set_rate
				    (ve_parent_pll_clk, arg_rate * 1000000)) {
					ve_parent_clk_rate =
					    clk_get_rate(ve_parent_pll_clk);
					if (clk_set_rate
					    (ve_moduleclk,
					     ve_parent_clk_rate)) {
	...
	case IOCTL_GET_ENV_INFO:
		{
			...
			if (copy_to_user
			    ((char *)arg, &env_info,
			     sizeof(struct cedarv_env_infomation)))
				return -EFAULT;
		}
		break;
...
static int cedardev_init(void)
{
	...
	ret =
	    request_irq(SUNXI_IRQ_VE, VideoEngineInterupt, 0, "cedar_dev",
			NULL);
	if (ret < 0) {
...
#error "Unknown chip type!"
	...
	setup_timer(&cedar_devp->cedar_engine_timer, cedar_engine_for_events,
		    (unsigned long)cedar_devp);
	setup_timer(&cedar_devp->cedar_engine_timer_rel,
		    cedar_engine_for_timer_rel, (unsigned long)cedar_devp);
